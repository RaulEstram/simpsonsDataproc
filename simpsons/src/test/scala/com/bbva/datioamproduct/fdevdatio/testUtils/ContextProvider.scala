...
trait ContextProvider extends FlatSpec with BeforeAndAfterAll with Matchers {
  ...
  val config: Config = new ProcessConfigLoader().fromPath("src/test/resources/config/simpsonsConfigTest.conf")
  val configError: Config = new ProcessConfigLoader().fromPath("src/test/resources/config/simpsonsErrorLauncherConfigTest.conf")
